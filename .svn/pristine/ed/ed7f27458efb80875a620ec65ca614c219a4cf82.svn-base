$(document).ready(function(){
	...
	$("#modifySavePassword").click(function(){
		...
		if(oldpwd==""){
			...
			tip.css("color","red");
			tip.html("对不起，您两次输入的密码不一致，请再次输入，谢谢！");
			$("#aginpassword").focus();
		}else{
			user=new Object();
			...
			$.ajax({
				...
				datatype:"html",
				timeout:1000,
				error:function(){alert("修改密码失败，请重试！");},
				success:function(result){
					if(result!="" && "successs"==result){
						tip.css("color","green");
						tip.html("修改密码成功 ，下次登录记得使用新密码哦。^_^");
					}else if(result=="failed"){
						tip.css("color","red");
						tip.html("修改密码失败，请重试！");
					}else if(result=="oldpwdwrong"){
						tip.css("color","red");
						tip.html("输入的旧密码有误，请重试！");
	...
	for(var i=0;i<json.length;i++){
		//config main menu
		result+='<li class="nav-header hidden-tablet" onclick="$(\'.test'+i+'\').toggle(500);" style="cursor:pointer">'+json[i].mainMenu.functionName+'</li>';
		for(var j=0;j<json[i].subMenus.length;j++){
			...
			switch(j){
				...
				default:pic="icon-picture";break;
			}
			result+='<li class="test'+i+'"><a class="ajax-link" href="'+json[i].subMenus[j].funcUrl+'" onclick="show_query_hint(\'query_hint\');" ><i class="'+pic+'"></i><span class="hidden-tablet">'+json[i].subMenus[j].functionName+'</span></a></li>';
		}
		/*result = result + "</ul></li>";*/
	}
	$("#menus").append(result);
