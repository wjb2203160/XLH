...
<div class="row-fluid sortable">
	<div class="box span12">
		<div class="box-header well" data-original-title>
			...
			<div class="box-icon">
				<span class="icon32 icon-color icon-add custom-setting adduser" id="intoAddUser"></span>
			</div>
		</div>		
...
<!-- 用户信息添加 -->
 <div class="modal hide fade" id="addUserDiv">
	<form action="/backend/addUser.htm" method="post" enctype="multipart/form-data" onsubmit="return addUserFunction();">
		...
		<div class="modal-body">
			...
			<ul class="topul">
				<li>
				<label>用户类型:</label> 
					<input id="seletusertypevname" type="hidden" name="valueName" value="" /> 
					<select id="selectusertype" name="userType" style="width: 100px;">
						<option value="" selected="selected">--请选择--</option>
							<c:if test="${userTypeList != null}">
								<c:forEach items="${userTypeList }" var="userType">
									<option value="${userType.id }">${userType.valueName }</option>
								</c:forEach>
							</c:if>
					</select> 
				...
				<li><label>性别：</label> <select name="gender" style="width: 100px;">
						<option value="" selected="selected">--请选择--</option>
						 ...
						 <select id="selectcardtype" name="cardTypeName" style="width: 100px;">
							...
							<c:if test="${cardTypeList != null}">
								<c:forEach items="${cardTypeList}" var="cardType">
									<option value="${cardType.id}">${cardType.valueName}</option>
								</c:forEach>
							</c:if>
						</select> 
					 <span style="color: red; font-weight: bold;">*</span>
				...
				<li><label>开户卡号：</label>
					<input type="text" id="a_bankaccount" name="bankAccount"
				...
				<li><label>Email：</label>
					<input type="text" id="a_email" name="email" /></li>
				<li><label>是否启用：</label> 
					<select name="isStart" style="width: 100px;">
						<option value="1" selected="selected">启用</option>
						<option value="2">不启用</option>
					</select> <span style="color: red; font-weight: bold;">*</span></li>
				<li class="lastli"><label>用户地址：</label> 
					<div class="container">
						<div data-toggle="distpicker">
							<select class="form-control" id="province1" style="width:120px"　></select>
							<select class="form-control" id="city1"  style="width:100px"></select>
							<select class="form-control" id="district1"  style="width:100px"></select>
						</div>
					</div>
			</ul>
			<div class="clear"></div>
			<ul class="downul">
				<li><label>上传身份证图片</label>
			...
			<ul class="downul">
				...
				<label>上传银行卡图片：</label>
					...
					<p><span style="color:red;font-weight: bold;">*注：1、大小不得超过50km。2、图片格式：jpg、png、jpeg、pneg</span></p>
